(() => {
  ...
  function detectCreator() {
    ...
    const enabled = query.get('creator') === '1' || localStorage.getItem('pixelDndCreator') === '1';
    if (enabled) localStorage.setItem('pixelDndCreator', '1');
    return enabled;
  }

  const isCreator = detectCreator();
  window.PixelDndAccess = {
    isCreator,
    creatorOnlyNames: [...creatorOnlyNames],
    canUse(name) {
      return isCreator || !creatorOnlyNames.some(item => String(name || '').toLowerCase().includes(item.toLowerCase()));
    }
  };

  async function loadBatches() {
    if (batches.length) return batches;
    const response = await fetch(manifestIndex, { cache: 'no-cache' });
    if (!response.ok) throw new Error(`Не загружен индекс Warhammer: ${response.status}`);
    const index = await response.json();
    batches = await Promise.all(index.manifests.map(async url => {
      const item = await fetch(url, { cache: 'no-cache' });
      if (!item.ok) throw new Error(`Не загружен манифест ${url}: ${item.status}`);
      return item.json();
    }));
    return batches;
  }
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .wh40k-nav{color:#efc66d!important}.wh40k-gallery{grid-column:1/-1;margin-top:12px;background:#090d13;border:1px solid #443a30;padding:12px}
      ...
      .wh40k-batch-label{grid-column:1/-1;color:#efc66d;border-bottom:1px solid #3a322a;padding:8px 2px 5px;font-size:11px}.wh40k-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:9px}
      .wh40k-card{background:#0c1119;border:1px solid #343d4d;padding:7px;color:#e7eaf0;text-align:left;cursor:pointer;overflow:hidden}.wh40k-card:hover{border-color:#d4ad5a;transform:translateY(-1px)}
      .wh40k-card-preview{display:block;width:100%;aspect-ratio:16/10;position:relative;overflow:hidden;background:#05070a;margin-bottom:7px}.wh40k-card-preview img{position:absolute;max-width:none}.wh40k-card-preview img.direct{inset:0;width:100%;height:100%;object-fit:cover}.wh40k-card b{display:block;font-size:9px}.wh40k-card em{display:block;color:#818b9a;font-size:8px;font-style:normal;margin-top:3px}
      .wh40k-loading{grid-column:1/-1;color:#9aa3b5;padding:20px;text-align:center;border:1px dashed #343d4d}.wh40k-locked{opacity:.45;filter:grayscale(1);pointer-events:none}
  ...
  function allowedAsset(asset) {
    ...
    if (asset.creator_only || asset.creatorOnly) return false;
    const label = `${asset.title_ru || ''} ${asset.title || ''} ${asset.faction || ''} ${asset.class || ''}`;
    return window.PixelDndAccess.canUse(label);
  }
  ...
  function createPreview(asset, batch) {
    ...
    img.alt = asset.title_ru || asset.asset_key || 'Warhammer asset';
    img.loading = 'lazy';
    ...
    if (!crop || !batch.sheet) {
      img.src = asset.url || batch.sheet?.url || image(asset.asset_key);
      img.className = 'direct';
      preview.appendChild(img);
      return preview;
    }
    const [x1,y1,x2,y2] = crop;
    const scale = Math.max(260 / (x2-x1), 163 / (y2-y1));
  ...
  async function renderGallery(section) {
    ...
    try {
      ...
      for (const batch of loaded) {
        const visibleAssets = (batch.assets || []).filter(allowedAsset);
        if (!visibleAssets.length) continue;
        const label = document.createElement('div');
        label.className = 'wh40k-batch-label';
        label.textContent = `${batch.title_ru || batch.batch_key || 'ПАРТИЯ'} · ${visibleAssets.length} АССЕТОВ`;
        ...
        for (const asset of visibleAssets) {
          ...
          meta.textContent = asset.category || 'ассет';
          card.append(title,meta);
          card.addEventListener('click',() => window.open(asset.url || batch.sheet?.url || image(asset.asset_key),'_blank','noopener'));
          grid.appendChild(card);
          total++;
        }
      }
      section.querySelector('[data-wh40k-count]').textContent = `${total} доступных ассетов${isCreator ? ' · аккаунт создателя' : ''}`;
    } catch (error) {
      console.error(error);
      grid.innerHTML = '<div class="wh40k-loading">Не удалось загрузить каталог. Повторите после развёртывания.</div>';
    }
  }

  function filterRestrictedOptions(root=document) {
    if (isCreator) return;
    root.querySelectorAll('option,[data-class],[data-character],[data-faction]').forEach(node => {
      const label = `${node.textContent || ''} ${node.dataset.class || ''} ${node.dataset.character || ''} ${node.dataset.faction || ''}`;
      if (!window.PixelDndAccess.canUse(label)) node.remove();
    });
  }

  function watchCharacterUi() {
    filterRestrictedOptions();
    if (isCreator || !document.body) return;
    const observer = new MutationObserver(records => records.forEach(record => record.addedNodes.forEach(node => {
      if (node.nodeType === 1) filterRestrictedOptions(node);
    })));
  ...
  function installCreator() {
    if (!isCreator || document.getElementById('wh40kCreatorButton')) return;
    ...
    panel.innerHTML = `<h3>Архитектор сектора</h3><div class="wh40k-cheats"><button>Восстановить отряд</button><button>Открыть карту</button><button>Вызвать босса</button><button>Изменить бросок</button><button>Аватар Примарха</button><button>Аватар Императора</button></div><p style="color:#aab1bf;font-size:9px">Закрытые персонажи: Император, Примарх, Сестра Битвы, Кустодий, Серый Рыцарь и Сестра Тишины.</p>`;
  ...
  function activate() {
    ...
    if (map) {
      map.style.backgroundImage = `linear-gradient(rgba(5,7,11,.25),rgba(5,7,11,.48)),url('${image('wh40k_maps')}')`;
